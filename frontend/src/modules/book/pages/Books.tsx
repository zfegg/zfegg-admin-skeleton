...
import {FormDrawer, ProColumnType, ProTable, valueEnumToOptions} from "@zfegg/admin-data-source-components";
...
const injection = injectServices((container) => {
    return {
        dataSource: container.get(Resources).create('book/books').createDataSource(),
    }
})
        ...
        return (
            <PageContainer extra={[
                <Button key={"add"} type={"primary"}
                        onClick={() => this.setState({visible: true, itemId: undefined})}>新增</Button>
            ]}>
                <ProTable
                    defaultSize={"small"}
                    columns={columns}
